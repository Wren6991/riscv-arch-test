...
class spike_simple(pluginTemplate):
    ...
    def runTests(self, testList):
      ...
      for testname in testList:
          ...
          compile_cmd = 'riscv{1}-unknown-elf-gcc -march={0} -mabi=ilp32' \
            ' -static -mcmodel=medany -fvisibility=hidden -nostdlib -nostartfiles -g' \
            ' -T ' + self.pluginpath + '/env/link.ld' \
            ' -I ' + self.pluginpath + '/env/' \
            ' -I ' + self.archtest_env + ' {2} -o {3}.elf {4}'

          cmd = compile_cmd.format(testentry['isa'].lower(), self.xlen, test, shortname, compile_macros)

          if self.target_run:
            simcmd = self.dut_exe + f' --isa={self.isa} +signature={sig_file} +signature-granularity=4 {shortname}.elf'
          else:
            simcmd = 'echo "NO RUN"'
